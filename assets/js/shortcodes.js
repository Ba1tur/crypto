...
; (function ($) {
    ...
    var tfCounter = function () {
        if ($(document.body).hasClass("counter-scroll")) {
            var a = 0;
            $(window).scroll(function () {
                var $tfCounter = $(".tf-counter");
                if ($tfCounter.length) { // Проверяем, существует ли элемент
                    var oTop = $tfCounter.offset().top - window.innerHeight;
                    if (a == 0 && $(window).scrollTop() > oTop) {
                        if ($().countTo) {
                            $tfCounter.find(".counter-number").each(function () {
                                var to = $(this).data("to"),
